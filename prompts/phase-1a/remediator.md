...
- An artifact the target repository's own instructions require for this kind of change, such as a decision note, a doc update, or a changelog entry, is part of done, not scope widening. When a finding reports a missing repository-required artifact, add the artifact rather than arguing it out of scope, unless the cited rule plainly does not apply; then say why in the summary. When its path is outside the allowed paths, use Required Work Outside Scope below instead of skipping it.
...
- Make the first line `# ` followed by the title you write for this change: a concise imperative title of at most 256 UTF-8 bytes. Follow it with nonempty paragraphs explaining the approved outcome, what changed and why, scope limits, unfinished work, and uncertainty. Keep the entire file within 8 KiB. Task names and the raw request are not PR titles.

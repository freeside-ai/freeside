...
identity='"lineage":{"campaign_id":"campaign-1","attempt_number":2,"kind":"retry","parent_run_id":"run-parent","source_digest":"sha256:source","approved_spec_digest":"sha256:spec","elaboration_run_id":"run-elab","implementation_run_id":"run-impl"},"admission":{"invocation_id":"inv-impl","stage":"implementation","image_ref":"registry/agent@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","image_digest":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","base":{"repo":"owner/repo","repository_id":42,"base_ref":"main","base_sha":"base-sha"},"trust_profile_digest":"sha256:profile","review_configuration_digest":"sha256:review-config"},"last_stage":"implementation"'
...
run_case() {
	local name=$1 elaboration=$2 timeout=$3 expected=$4
	local case_dir=$fixture_root/$name status
	mkdir -p "$case_dir"
	set +e
	real_work_supervise "$stub" /state/freeside.db "$elaboration" run-impl "" \
...
run_case success run-elab 3 0
assert_contains "$fixture_root/success/stderr" 'elaboration run=run-elab state=waiting_for_specification_approval'
...
# Rejection and elaboration execution failure are terminal immediately and
...
write_sequence run-elab "$(snapshot failed failed "$rejected_extra")"
run_case rejection run-elab 3 1
...
write_sequence run-elab "$(snapshot failed failed "$failure_extra")"
run_case elaboration-failure run-elab 3 1
assert_contains "$fixture_root/elaboration-failure/stderr" 'attention=execution-failure-1 type=execution_failure'
...
run_case approval-wait run-elab 3 0
...
write_sequence run-elab "$(snapshot implementation_bound pending)"
write_sequence run-impl "$(snapshot failed failed ',"terminal":"failed"')"
run_case implementation-failure run-elab 3 1
...
write_sequence run-impl \
	...
	"$(snapshot attention_required pending "$parked_extra")" \
	"$(snapshot published published)"
run_case parked-review run-elab 3 0
...
# or resubmits the elaboration lane.
rm -f "$stub_root/run-elab.count"
write_sequence run-impl "$(snapshot published published)"
run_case resume "" 3 0
if [[ -e "$stub_root/run-elab.count" ]]; then
	echo "resume observed the elaboration lane" >&2
	exit 1
fi

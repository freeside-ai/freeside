...
Round 31 raised one P2 (fixed) and one P1 (`accepted-by-decision`, declined):

- *P2: list/inspect/export CLI errors folded untrusted runtime stderr into the
  reason.* Rounds 24-25 kept a bounded stderr tail in CLI-call errors for
  diagnosis on the assumption non-create calls carry only gate-generated
  identifiers. But `list` enumerates every runtime object and `inspect`/`export`
  can touch a foreign object recycled into the deterministic name, so a
  runtime/XPC message could name a user-created object, and these errors are
  wrapped into `ConformanceFailure.Reason`, which the identity-redaction class
  (round 27) requires to carry no runtime-observed identity. `run` (list/inspect)
  and `ExportRootFS` now withhold stderr (`reportStderr=false`), returning the
  operation and safe exit status only; the bounded tail stays for mutations on
  the gate's own names (volume/container create/delete, start/stop), whose stderr
  references only gate-generated identifiers. This revises the round-27
  "stderr-tail, rejected-by-verification" note: the specific list-names-a-foreign
  -object vector is real. Regression: `TestListInspectExportWithholdStderr`.
- *P1: the success-path export binds to a mutable name, not the inspected
  container.* `ExportRootFS(names.Exporter)` targets the deterministic name, so a
  foreign actor that recycles it between check 4's approving inspect and the
  export can have its rootfs streamed and released; the proof is static markers
  (`workspace_mounted=ro`, `credentials=absent`, ...) with no per-run secret, so a
  self-consistent foreign proof/manifest/blob tree passes checks 5/7. This is the
  success-path sibling of the #138 mutable-name class: closing it needs either the
  immutable-id `Runtime` primitive (#138) or binding the proof to an unforgeable
  per-run nonce. The exporter image and command are configured, not
  gate-constructed, and `buildExporterSpec` injects no env, so the nonce path is a
  cross-contract change to the exporter payload, outside the ward unit's scope.
  Declined here and folded into #138's scope, which now covers success-path export
  binding as well as cleanup; merge-readiness stays gated on it.

Round 32 raised one P2 (fixed): a panic after the output dir was created (for
example a typed-nil `OutputScanner` that passes the non-nil config check but
panics when called) left the unscanned output on the host, because the deferred
cleanup removed `exportDir` only when the named `err` was non-nil, and `err` is
still nil while a panic unwinds. `runState` now carries an explicit `succeeded`
flag set only immediately before the successful return, and the cleanup keeps the
output dir only when `succeeded && err == nil`; every other unwind, including a
panic and a teardown failure that nils a good result, removes it. Regression:
`TestHandoffPanicRemovesOutput` proves a panicking scanner leaves no scratch dir,
and fails against the prior err-keyed cleanup.

...
func (s *ReviewSource) Inspect(_ context.Context, id domain.InvocationID) (exec.Status, error) {
	...
	case OutcomeCrashAfterResult:
		...
		return exec.StatusGone, nil
	case OutcomeBlocked, OutcomeCancel:
		// A review never stops on a question (that outcome belongs to the stage
		// driver) and never ends on a scripted cancellation, so either in a
		// review script is a fixture defect.

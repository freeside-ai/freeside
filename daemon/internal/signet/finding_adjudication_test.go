...
func TestSubmitFindingAdjudicationAcceptAndReplay(t *testing.T) {
	...
	decision, err := signet.DecodeFindingAdjudicationDecision(result.Record, item)
	if err != nil || decision.Action != domain.ActionAcceptRecommendedRoute || len(decision.AlternativeChoices) != 0 {
		t.Fatalf("decoded decision = %+v, %v", decision, err)
	}
	...
	decision, err = signet.DecodeFindingAdjudicationDecision(result.Record, decided)
	if err != nil || decision.Action != domain.ActionAcceptRecommendedRoute {
		t.Fatalf("decode against terminal successor = %+v, %v", decision, err)
	}
	drifted := decided
	drifted.ItemVersion++
	if _, err := signet.DecodeFindingAdjudicationDecision(result.Record, drifted); !errors.Is(err, signet.ErrInvalidFindingAdjudicationDecisionPayload) {
		t.Fatalf("decode against later successor = %v, want invalid payload", err)
	}
	malformedCommand := result.Record
	malformedCommand.CommandID = ""
	if _, err := signet.DecodeFindingAdjudicationDecision(malformedCommand, decided); !errors.Is(err, signet.ErrInvalidFindingAdjudicationDecisionPayload) {
		t.Fatalf("decode malformed command = %v, want invalid payload", err)
	}
	malformedItem := decided
	malformedItem.FindingAdjudication = &domain.FindingAdjudicationBinding{
		RunID:              decided.FindingAdjudication.RunID,
		Round:              decided.FindingAdjudication.Round,
		AdjudicationDigest: decided.FindingAdjudication.AdjudicationDigest,
		Proposals:          append([]domain.FindingAdjudicationProposal{}, decided.FindingAdjudication.Proposals...),
	}
	malformedItem.FindingAdjudication.Proposals[0].OfferedAlternatives = []domain.OfferedAlternative{{
		Route: domain.RouteDefer, Consequence: "not compatible with a contradictory finding",
	}}
	if _, err := signet.DecodeFindingAdjudicationDecision(result.Record, malformedItem); !errors.Is(err, signet.ErrInvalidFindingAdjudicationDecisionPayload) {
		t.Fatalf("decode malformed item = %v, want invalid payload", err)
	}
...
func TestSubmitFindingAdjudicationAlternativeChoice(t *testing.T) {
	...
	decision, err := signet.DecodeFindingAdjudicationDecision(result.Record, item)
	if err != nil {
		t.Fatalf("DecodeFindingAdjudicationDecision: %v", err)
	}
	if len(decision.AlternativeChoices) != 2 || decision.AlternativeChoices[0].FindingID != "finding-a" {
		t.Fatalf("choices = %+v, want canonical finding order", decision.AlternativeChoices)
	}
	decided, _ := f.itemSnapshotFor(t, item.ID)
	decision, err = signet.DecodeFindingAdjudicationDecision(result.Record, decided)
	if err != nil || len(decision.AlternativeChoices) != 2 {
		t.Fatalf("decode against terminal successor = %+v, %v", decision, err)
	}
}

func TestDecodeFindingAdjudicationDecisionRejectsUnofferedActions(t *testing.T) {
	tests := []struct {
		name   string
		action domain.Action
		choose bool
	}{
		{"accept recommended route", domain.ActionAcceptRecommendedRoute, false},
		{"choose alternative route", domain.ActionChooseAlternativeRoute, true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			f := newFixture(t)
			item := seedFindingAdjudicationItem(t, f)
			command := commandOn(item, "command-unoffered-"+string(test.action), test.action)
			if test.choose {
				command.Payload.AlternativeChoices = []signet.AlternativeChoice{{
					FindingID: "finding-a", Route: domain.RouteDispute,
				}}
			...
			for _, state := range []struct {
				...
				{"open", item},
				{"resolved successor", decided},
			} {
				t.Run(state.name, func(t *testing.T) {
					unoffered := state.item
					unoffered.RequestedDecision = nil
					for _, offered := range state.item.RequestedDecision {
						if offered != test.action {
							unoffered.RequestedDecision = append(unoffered.RequestedDecision, offered)
						}
					}
					if err := unoffered.Validate(); err != nil {
						t.Fatalf("unoffered item is not structurally valid: %v", err)
					}
					if _, err := signet.DecodeFindingAdjudicationDecision(result.Record, unoffered); !errors.Is(err, signet.ErrInvalidFindingAdjudicationDecisionPayload) {
						t.Fatalf("DecodeFindingAdjudicationDecision = %v, want invalid payload", err)
					}
				})
			}
		})

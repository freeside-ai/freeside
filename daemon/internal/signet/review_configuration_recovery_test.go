...
// TestSubmitReviewConfigurationRecoveryRejectsIneffectiveTarget pins the
// decision-time effectiveness gate (issue #611, Codex round 4): adopting
// while the latest activated revision does not approve the daemon's
// effective reviewer configuration rejects the command and rolls the whole
// transaction back (no revision, item still open, no transition), because
// accepting would conclude the item while the one-adoption-per-failure
// binding blocks any corrected retry. Activating the matching revision and
// resubmitting then succeeds.
func TestSubmitReviewConfigurationRecoveryRejectsIneffectiveTarget(t *testing.T) {
	ctx := context.Background()
	f := newFixture(t)
	item := seedReviewConfigurationRecoveryItem(t, f)
	effective := domain.Digest("sha256:config-effective")
	gated := signet.NewService(f.store,
		signet.WithClock(func() time.Time { return *f.now }),
		signet.WithEffectiveReviewConfiguration(func() domain.Digest { return effective }),
	)
	before := f.revision(t)

	_, err := gated.Submit(ctx, commandOn(item, "command-adopt-ineffective", domain.ActionAdoptReviewConfiguration))
	if !errors.Is(err, domain.ErrReviewConfigAdoptionIneffective) {
		t.Fatalf("Submit(ineffective target) = %v, want %v",
			err, domain.ErrReviewConfigAdoptionIneffective)
	}
	if after := f.revision(t); after != before {
		t.Fatalf("rejected adoption moved the revision %d -> %d", before, after)
	}
	if err := f.store.Read(ctx, func(tx *store.ReadTx) error {
		...
		if got.Status != domain.StatusOpen {
			t.Fatalf("rejected adoption concluded the item: %#v", got)
		}
		_, found, err := tx.LatestReviewConfigurationRecoveryTransition(
			ctx, item.ReviewConfigurationRecovery.RunID)
		if err != nil {
			return err
		}
		if found {
			t.Fatal("rejected adoption recorded a transition")
		}
		return nil
	}); err != nil {
		t.Fatal(err)
	...
	// The operator activates the matching revision; the retry is accepted.
	matching := configRecoveryTestProfile(t, effective)
	if err := f.store.WriteInternal(ctx, func(tx *store.InternalTx) error {
		return tx.RecordTrustProfile(ctx, matching, f.now.Add(time.Minute))
	}); err != nil {
		t.Fatalf("activate matching revision: %v", err)
	}
	if _, err := gated.Submit(ctx,
		commandOn(item, "command-adopt-effective", domain.ActionAdoptReviewConfiguration),
	); err != nil {
		t.Fatalf("Submit(matching target): %v", err)
	}
	if err := f.store.Read(ctx, func(tx *store.ReadTx) error {
		transition, found, err := tx.LatestReviewConfigurationRecoveryTransition(
			ctx, item.ReviewConfigurationRecovery.RunID)
		...
		if !found || transition.SupersedingProfileDigest != matching.ProfileDigest {
			t.Fatalf("accepted retry transition = %+v (found %v), want superseding %s",
				transition, found, matching.ProfileDigest)
		}
		return nil
	}); err != nil {
		t.Fatal(err)

...
// adjudicator's allowed-free proposal lattice. UsesEngineCompatibility marks
// the pre-composition required row whose compatibility and route are supplied
// by the engine after the model response; it adds no ninth domain row.
...
func equalAdjudicationRows(left, right []AdjudicationRow) bool {
	return slices.EqualFunc(left, right, func(a, b AdjudicationRow) bool {
		if a.GoalRelationship != b.GoalRelationship || a.Route != b.Route ||
			a.UsesEngineCompatibility != b.UsesEngineCompatibility {
			return false
		}

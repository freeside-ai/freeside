...
// or the newest one records a failure. It is deliberately distinct from
// ErrNotFound: the admission was refused, which is not the same as having no
// record to return.
...
// proved. The newest row per backend is its current declaration and the row
// id is the proof generation, mirroring ward's in-memory generation guard: a
// newer append (any outcome) supersedes the older record, and a failed
// append invalidates the declaration. Rows are daemon-internal and written
// by the suite runner through InternalTx, so they carry no
// entity_version/as_of_revision and never bump the server revision.
const (
	recordBackendConformanceSQL = `
INSERT INTO backend_conformance_records (backend, outcome, capabilities, proved_at)
VALUES (?, ?, ?, ?)`
...
SELECT id, outcome, capabilities, proved_at
...
// record must arrive unpersisted (Generation zero): the generation is this
...
func (tx *InternalTx) RecordBackendConformance(
	...
	result, err := tx.tx.ExecContext(ctx, recordBackendConformanceSQL,
		string(record.Backend), string(record.Outcome), string(capabilities),
...
func (tx *ReadTx) LatestBackendConformance(
	...
	err := tx.tx.QueryRowContext(ctx, latestBackendConformanceSQL, string(backend)).
		Scan(&id, &outcome, &capabilities, &provedAt)
	switch {

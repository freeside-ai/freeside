...
func (l *RuntimeCodexReviewVolumeLeaser) AcquireCodexReviewVolumeLease(
	...
	if holder == "" || len(volumes) != 2 || volumes[0] == volumes[1] {
...
func (l *RuntimeCodexReviewVolumeLeaser) reconstructTransferLocked(
	...
	for _, container := range containers {
		...
		attached := make([]string, 0, len(volumes))
		volumeMounts := 0
		...
		for _, mount := range report.Mounts {
			...
			volumeMounts++
			if !slices.Contains(volumes, mount.Source) || slices.Contains(attached, mount.Source) {
				invalidAttachment = true
				continue
			}
			attached = append(attached, mount.Source)
		}
		if len(attached) == 0 {
			continue
		}
		if invalidAttachment || volumeMounts != len(volumes) || len(attached) != len(volumes) || !report.LabelsObserved ||

...
// wait is budgeted in whole poll intervals (timeout / PollInterval attempts),
// so tests with an injected Sleep are fully deterministic; its own deadline
// also bounds each runtime call, so a wedged Inspect cannot defeat the named

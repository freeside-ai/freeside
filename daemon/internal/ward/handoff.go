...
// budgeted in whole poll intervals (ceil(timeout / PollInterval) attempts), so
// tests with an injected Sleep are fully deterministic; rounding up keeps a
// timeout that is shorter than, or not a whole multiple of, the interval from
// dropping its final partial interval and giving up a poll early. Its own
// deadline also bounds each runtime call: a wedged Inspect must not defeat the
// named writer or exporter timeout when the caller context has no deadline.

...
// TestListInspectExportWithholdStderr proves the calls that touch objects
// beyond the gate's own deterministic names (list, inspect, export) never fold
// the runtime's stderr into their error: a runtime/XPC message can name a
// foreign user-created object, and these errors reach ConformanceFailure.Reason.
func TestListInspectExportWithholdStderr(t *testing.T) {
	bin := filepath.Join(t.TempDir(), "container-fixture")
	if err := os.WriteFile(bin, []byte("#!/bin/sh\nprintf 'foreign-container-SECRET\\n' >&2\nexit 7\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.Chmod(bin, 0o700); err != nil { //nolint:gosec // executable test fixture is isolated in t.TempDir
		t.Fatal(err)
	...
	const foreign = "foreign-container-SECRET"
	ctx := context.Background()
	calls := map[string]func() error{
		"inspect":         func() error { _, err := rt.Inspect(ctx, "gate-name"); return err },
		"list containers": func() error { _, err := rt.ListContainers(ctx); return err },
		"list volumes":    func() error { _, err := rt.ListVolumes(ctx); return err },
		"export":          func() error { return rt.ExportRootFS(ctx, "gate-name", &bytes.Buffer{}, 0) },
	}
	for name, call := range calls {
		t.Run(name, func(t *testing.T) {
			err := call()
			if err == nil {
				t.Fatal("call returned nil, want exit error")
			}
			if strings.Contains(err.Error(), foreign) {
				t.Errorf("%s error folded runtime stderr naming a foreign object: %v", name, err)
			}
			if !strings.Contains(err.Error(), "exit status 7") {
				t.Errorf("%s error lost the safe exit status: %v", name, err)
			}
		})
	}
}

...
| `ready_for_final_review` | view PR, return to agent, mark seen, dismiss, stop | Done: the card leads with typed diff stats over the readiness checklist and per-round yield composition; change summary remains data-gated and return to agent is omitted until #919 | Remaining readiness display below |
...
| `blocked` | read only | Done: the card and row lead with the typed wait, its start, and the blocking item or pull request, daemon facts only | |

...
| Run list | Done | Active / Finished / All scopes mirror the Inbox selector, default to Active, and show counts within the project filter. Lifecycle controls membership; every scope orders by newest activity. Explicit run links reveal the target scope; changing a filter repairs selection and navigation. A row's title is the current stage and round; the meta line under it carries the project, the work unit when named, when the run was last active, and the armed watch chips. The last-active segment is the instant the list orders by, so the order reads off the cards: a coarse relative time under a day (`last active 30m ago`), the abbreviated date and short time from a day on (`last active Sep 4, 3:14 PM`), and nothing at all for a run with no recorded observation; macOS hover carries the exact instant in ISO 8601, and a live row re-reads its own clock every minute. The submission instant stays on the run timeline as its `run_submitted` milestone. A four-dot stage rail (specification, implementation, review, verification, plus any stage the daemon recorded under another name) marks stages left behind as completed, the last recorded stage by outcome and lifecycle, and finished or unobserved runs with no current dot; the dot names are spoken by VoiceOver and shown on hover. Rows also name the campaign attempt and successor, merged PR on completion, and billable spend so far. An approved specification handoff explicitly names implementation, even when the successor is absent from the list, and its pending specification stage reads completed. Retry supersession keeps its later-attempt wording; pending or blocked finished implementation stages stay pending rather than claiming success. Selected rows use the same leading bar and wash as the inbox. Current holds or milestones remain visible. §15 outcome chips keep the daemon vocabulary (ready and merged are quiet, blocked accent, failed wax, not observed dashed); a run in progress shows no chip. With no runs in scope, macOS centers the empty state (glyph, title, description) in the area below the filters, which stay pinned at the top; iPhone shows the same message centered in the column (ContentUnavailableView, also kept in the detail pane). |
| Run timeline | Done | The one-line title is the attempt number (the run id when the campaign or attempt number is absent) under a `Run timeline · run-id` eyebrow that keeps the id's spelling for selection and the copy context menu. The full campaign id appears in the header details with text selection and a Copy campaign ID context menu. At accessibility text sizes, the outcome chip sits below the heading. Reviewer output leads with the reported conclusion, labeled as an agent claim beside separate daemon facts. The Review section lists review rounds newest round first. The reader shows the final message, command activity, and labeled diagnostics, with exact retained output in a secondary raw disclosure. Missing or unreadable results never read as clean. The Stage, Round & Decision History rail lists the newest milestone first, with the top entry marked current; the client reverses the daemon's record order rather than sorting by timestamp, so equal-timestamp entries stay in reverse recording order. Invocation observations group under the stage or review round whose attempt produced them; rows order by attempt (newest first) and groups by the start of their newest attempt, so a late re-observation moves only a row's observed time, never its place. Each row and rail entry reads `<Stage> · Round <n>` with the daemon's per-stage round; when a run recorded the same stage more than once (the daemon appends a fresh implementation stage per remediation or operator-feedback pass), the label also names its pass in stage creation order (`Implementation · Pass 2 · Round 1`); a review round stays `Review · Round <n>`. The observed time is freshness only. Observations matching no attempt fall under Unattributed, which sorts last. §15 styling: accent-washed hold card, milestone rail, invocation status chips. A loaded timeline stays on screen across routine refreshes and bootstraps (a live run bootstraps every round); the spinner and the unavailable state appear only before the first load or when the first load fails. |
...
- The "More actions ▾" and route-picker popups, and the project `Picker(.menu)`.

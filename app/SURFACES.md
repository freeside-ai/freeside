...
| Run list | Done | Active / Finished / All scopes mirror the Inbox selector, default to Active, and show counts within the project filter. Lifecycle controls membership; every scope orders by newest activity. Explicit run links reveal the target scope; changing a filter repairs selection and navigation. Rows name the work item, stage, round, campaign attempt and successor, merged PR on completion, and billable spend so far. Selected rows use the same leading bar and wash as the inbox. Current holds or milestones and wrapping watch chips remain visible. §15 outcome chips keep the daemon vocabulary (ready and merged are quiet, in progress water, blocked accent, failed wax, not observed dashed). |

...
| Run timeline | Done | The one-line title is the attempt number (the run id when the campaign or attempt number is absent) under a `Run timeline · run-id` eyebrow that keeps the id's spelling for selection and the copy context menu. The full campaign id appears in the header details with text selection and a Copy campaign ID context menu. At accessibility text sizes, the outcome chip sits below the heading. Reviewer output leads with the reported conclusion, labeled as an agent claim beside separate daemon facts. The reader shows the final message, command activity, and labeled diagnostics, with exact retained output in a secondary raw disclosure. Missing or unreadable results never read as clean. Invocation observations group under the stage whose attempt produced them, groups and rows newest first, with observations matching no attempt under Unattributed. §15 styling: accent-washed hold card, milestone rail, invocation status chips. A loaded timeline stays on screen across routine refreshes and bootstraps (a live run bootstraps every round); the spinner and the unavailable state appear only before the first load or when the first load fails. |

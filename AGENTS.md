...
| `images/`     | OCI images     | `bash scripts/build-exporter-image.sh`; `bash scripts/build-agent-claude-image.sh` then `bash scripts/check-agent-image.sh <ref>`; `bash scripts/build-project-gh-imgup-image.sh` then `bash scripts/check-agent-image.sh <ref>` (a project image is an agent image too) and `bash scripts/check-project-offline.sh <ref>` (all need Apple `container`, and the builds need container egress); `agent-codex` not yet initialized |
| `scripts/`    | Bash           | `bash -n scripts/*.sh app/scripts/*.sh`; `shellcheck scripts/*.sh app/scripts/*.sh`; `bash scripts/test-merge-result-audit.sh` (CI pins shellcheck in `.github/workflows/scripts-ci.yml`) |
